...
export const CostCodeListComponent = ({
  permissions,
  tableData = [],
  pages,
  handlePage,
  ...
  return (
    <>
      <Header as="h2" floated="left">
        <Icon name="suitcase" />
        <Header.Content>
          {t('costcode')} <Header.Subheader>{t('list')}</Header.Subheader>
        </Header.Content>
      </Header>
      {(permissions.admin || permissions.account_create) && (
        <Header floated="right">
          <Header.Content>
            <Link href="/chartofaccount/costcode/create">
              <Button color="blue">
                <Icon name="plus" /> {t('add_button_costcode')}
              </Button>
            </Link>
          </Header.Content>
        </Header>
        ...
        <Table.Body>
          {tableData.map(value => (
            <Table.Row key={value._id}>
              ...
              <Table.Cell>
                <Link href={`/chartofaccount/costcode/detail/${value._id}`}>
                  <Button icon>
                    <Icon name="file alternate outline" />
                  </Button>
                </Link>
                {(permissions.admin || permissions.account_edit) && (
                  <Link href={`/chartofaccount/costcode/edit/${value._id}`}>
                    <Button icon>
                      <Icon name="edit" />
                    </Button>
                  </Link>
