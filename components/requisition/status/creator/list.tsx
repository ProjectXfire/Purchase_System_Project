...
export const CreatorStatusListComponent = ({
  permissions,
  tableData = [],
  pages,
  handlePage,
  ...
  return (
    <>
      <Header as="h2" floated="left">
        <Icon name="suitcase" />
        <Header.Content>
          Creator Status<Header.Subheader>List</Header.Subheader>
        </Header.Content>
      </Header>
      {permissions.admin && (
        <Header floated="right">
          <Header.Content>
            <Link href="/requisition/status/creator/create">
              <Button color="blue">
                <Icon name="plus" /> Add creator status
              </Button>
            </Link>
          </Header.Content>
        </Header>
        ...
        <Table.Body>
          {tableData.map(value => (
            <Table.Row key={value._id}>
              ...
              <Table.Cell>
                <Link href={`/requisition/status/creator/detail/${value._id}`}>
                  <Button icon>
                    <Icon name="file alternate outline" />
                  </Button>
                </Link>
                {permissions.admin && (
                  <Link href={`/requisition/status/creator/edit/${value._id}`}>
                    <Button icon>
                      <Icon name="edit" />
                    </Button>
                  </Link>
