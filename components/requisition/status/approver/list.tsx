...
export const ApproverStatusListComponent = ({
  permissions,
  tableData = [],
  pages,
  handlePage,
  ...
  return (
    <>
      <Header as="h2" floated="left">
        <Icon name="suitcase" />
        <Header.Content>
          Approver Status<Header.Subheader>List</Header.Subheader>
        </Header.Content>
      </Header>
      {permissions.admin && (
        <Header floated="right">
          <Header.Content>
            <Link href="/requisition/status/approver/create">
              <Button color="blue">
                <Icon name="plus" /> Add approver status
              </Button>
            </Link>
          </Header.Content>
        </Header>
        ...
        <Table.Body>
          {tableData.map(value => (
            <Table.Row key={value._id}>
              ...
              <Table.Cell>
                <Link href={`/requisition/status/approver/detail/${value._id}`}>
                  <Button icon>
                    <Icon name="file alternate outline" />
                  </Button>
                </Link>
                {permissions.admin && (
                  <Link href={`/requisition/status/approver/edit/${value._id}`}>
                    <Button icon>
                      <Icon name="edit" />
                    </Button>
                  </Link>
