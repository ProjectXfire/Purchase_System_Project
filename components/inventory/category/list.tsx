  ...
  return (
    <>
      <Header as="h2" floated="left">
        <Icon name="suitcase" />
        <Header.Content>
          Category<Header.Subheader>List</Header.Subheader>
        </Header.Content>
      </Header>
      {permissions.admin && (
        <Header floated="right">
          <Header.Content>
            <Link href="/inventory/category/create">
              <Button color="blue">
        ...
        <Table.Body>
          {tableData.map(value => (
            <Table.Row key={value._id}>
              ...
              <Table.Cell>
                <Link href={`/inventory/category/detail/${value._id}`}>
                  <Button icon>
                    <Icon name="file alternate outline" />
                  </Button>
                </Link>
                {permissions.admin && (
                  <Link href={`/inventory/category/edit/${value._id}`}>
                    <Button icon>
                      <Icon name="edit" />
                    </Button>
                  </Link>
