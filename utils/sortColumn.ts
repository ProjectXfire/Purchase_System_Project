export const sortColumn = (array: any[], columnSelected: string): any[] => {
  ...
  if (prop.length === 2) {
    columnSorted = columnSorted.slice(0).sort((before: any, after: any) => {
      before = before[prop[0]] ? before[prop[0]][prop[1]] : ''
      after = after[prop[0]] ? after[prop[0]][prop[1]] : ''
      if (before.toLowerCase() > after.toLowerCase()) {
        return 1
      }
      if (before.toLowerCase() < after.toLowerCase()) {
  ...
  if (prop.length === 3) {
    columnSorted = columnSorted.slice(0).sort((before: any, after: any) => {
      before = before[prop[0]] ? before[prop[0]][prop[1]][prop[2]] : ''
      after = after[prop[0]] ? after[prop[0]][prop[1]][prop[2]] : ''
      if (before.toLowerCase() > after.toLowerCase()) {
        return 1
      }
      if (before.toLowerCase() < after.toLowerCase()) {
