...
import { ShipToListComponent } from '@components/requisition/shipto/list'
...
export const getServerSideProps: GetServerSideProps = async (
  ...
  const response = await getListSSR('requisition/ship-to/list?page=1', ctx)
  return {
    props: response
  }
}

const ShipToListPage = ({
  user,
  token,
  permissions,
  data,
  ...
  const handlePage = async (activePage: number) => {
    try {
      const response = await getList(
        `requisition/ship-to/list?page=${activePage}`,
        token
      )
  ...
  const deleteItem = async () => {
    try {
      await deleteOne('requisition/ship-to/delete', selectedItem.itemId, token)
      setSelectedItem({
        itemId: '',
        itemName: ''
      })
