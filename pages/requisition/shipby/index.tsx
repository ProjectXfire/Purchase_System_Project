...
import { ShipByListComponent } from '@components/requisition/shipby/list'
...
export const getServerSideProps: GetServerSideProps = async (
  ...
  const response = await getListSSR('requisition/ship-by/list?page=1', ctx)
  return {
    props: {
      ...response,
...
const ShipByListPage = ({
  ...
  const handlePage = async (activePage: number) => {
    try {
      const response = await getList(
        `requisition/ship-by/list?page=${activePage}`,
        token
      )
  ...
  const deleteItem = async () => {
    try {
      await deleteOne('requisition/ship-by/delete', selectedItem.itemId, token)
      setSelectedItem({
        itemId: '',
        itemName: ''
      })
