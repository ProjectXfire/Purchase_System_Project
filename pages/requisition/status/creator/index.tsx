...
import { CreatorStatusListComponent } from '@components/requisition/status/creator/list'
...
export const getServerSideProps: GetServerSideProps = async (
  ...
  const response = await getListSSR(
    'requisition/creator-status/list?page=1',
    ctx
  )
  return {
    props: response
...
const CreatorStatusListPage = ({
  ...
  const handlePage = async (activePage: number) => {
    try {
      const response = await getList(
        `requisition/creator-status/list?page=${activePage}`,
        token
      )
  ...
  const deleteItem = async () => {
    try {
      await deleteOne(
        'requisition/creator-status/delete',
        selectedItem.itemId,
        token
      )
      setSelectedItem({
...
export default CreatorStatusListPage
