...
import { CategoryListComponent } from '@components/inventory/category/list'
...
export const getServerSideProps: GetServerSideProps = async (
  ...
  const response = await getListSSR(
    'requisition/item-category/list?page=1',
    ctx
  )
  return {
    props: response
...
const CategoryListPage = ({
  ...
  const handlePage = async (activePage: number) => {
    try {
      const response = await getList(
        `requisition/item-category/list?page=${activePage}`,
        token
      )
  ...
  const deleteItem = async () => {
    try {
      await deleteOne(
        'requisition/item-category/delete',
        selectedItem.itemId,
        token
      )
      setSelectedItem({
