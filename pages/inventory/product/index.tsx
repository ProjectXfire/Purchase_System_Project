...
import { ProductListComponent } from '@components/inventory/product/list'
...
export const getServerSideProps: GetServerSideProps = async (
  ...
  const response = await getListSSR('requisition/item/list?page=1', ctx)
  return {
    props: response
  }
}
...
const ProductListPage = ({
  ...
  const handleSearchedValues = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    setDataModified(
      searchItems(initialData, ['partNumber', 'description'], value)
    )
  }
  ...
  const handlePage = async (activePage: number) => {
    try {
      const response = await getList(
        `requisition/item/list?page=${activePage}`,
        token
      )
  ...
  const deleteItem = async () => {
    try {
      await deleteOne('requisition/item/delete', selectedItem.itemId, token)
      setSelectedItem({
        itemId: '',
        itemName: ''
      })
