...
import { ContractListComponent } from '@components/contract/contract/list'
...
export const getServerSideProps: GetServerSideProps = async (
  ...
  const response = await getListSSR('contract/list?page=1', ctx)
  return {
    props: response
  }
}
...
const ContractListPage = ({
  ...
  const handlePage = async (activePage: number) => {
    try {
      const response = await getList(`contract/list?page=${activePage}`, token)
  ...
  const deleteItem = async () => {
    try {
      await deleteOne('contract/delete', selectedItem.itemId, token)
      setSelectedItem({
        itemId: '',
        itemName: ''
      })
