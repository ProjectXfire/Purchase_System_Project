...
import { ExpenseAccountListComponent } from '@components/generalexpense/expense-account/list'
...
export const getServerSideProps: GetServerSideProps = async (
  ...
  const response = await getListSSR('expense/expense-account/list?page=1', ctx)
  return {
    props: response
  }
}

const ExpenseAccountListPage = ({
  user,
  token,
  permissions,
  data,
  ...
  const handlePage = async (activePage: number) => {
    try {
      const response = await getList(
        `expense/expense-account/list?page=${activePage}`,
        token
      )
...
export default ExpenseAccountListPage
