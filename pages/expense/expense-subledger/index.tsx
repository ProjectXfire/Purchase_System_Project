...
import { ExpenseSubledgerListComponent } from '@components/generalexpense/expense-subledger/list'
...
export const getServerSideProps: GetServerSideProps = async (
  ...
  const response = await getListSSR(
    'expense/expense-subledger/list?page=1',
    ctx
  )
  return {
    props: response
...
const ExpenseSubledgerListPage = ({
  user,
  token,
  permissions,
  data,
  ...
  const handlePage = async (activePage: number) => {
    try {
      const response = await getList(
        `expense/expense-subledger/list?page=${activePage}`,
        token
      )
...
export default ExpenseSubledgerListPage
