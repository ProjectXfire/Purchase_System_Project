...
import { ExpenseListComponent } from '@components/generalexpense/expense/list'
...
export const getServerSideProps: GetServerSideProps = async (
  ...
  const response = await getListSSR('expense/list?page=1', ctx)
  return {
    props: response
  }
}
...
const ExpenseListPage = ({
  ...
  const handlePage = async (activePage: number) => {
    try {
      const response = await getList(`expense/list?page=${activePage}`, token)
  ...
  const deleteItem = async () => {
    try {
      await deleteOne('expense/delete', selectedItem.itemId, token)
      setSelectedItem({
        itemId: '',
        itemName: ''
      })
